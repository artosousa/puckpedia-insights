...
export const printBrandedReport = (title: string, sections: { title: string; rows: ExportRow[] }[], branding: ExportBranding) => {
  ...
  const tables = sections.map((section) => {
    ...
    return `<section><h2>${escapeHtml(section.title)}</h2><table><thead><tr>${headers.map((key) => `<th>${escapeHtml(key.replaceAll("_", " "))}</th>`).join("")}</tr></thead><tbody>${body}</tbody></table></section>`;
  }).join("");
